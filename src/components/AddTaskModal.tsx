...
import { X, Plus, Calendar, Sparkles, BookOpen, Clock, Dumbbell } from 'lucide-react';
...
export const AddTaskModal: React.FC<AddTaskModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const studySessions =
      generatePlan || type === 'teste'
        ? generateStudyPlan(sub?.name || subjectCode, dueDate, sessionsCount)
        : undefined;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 bg-slate-900/60 backdrop-blur-xs animate-in fade-in">
      <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl border border-slate-100 overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="p-5 overflow-y-auto space-y-4">
          ...
          {/* Study Plan Generator Checkbox (Especially for tests) */}
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-3.5 space-y-2.5">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                checked={generatePlan || type === 'teste'}
